...
class FirebaseService(private val database: DatabaseReference) {
    ...
    fun setScreenData(screenId: String, data: Any) {
        // Write under the same "screens/{screenId}" path that readers use
        android.util.Log.d("FirebaseService", "Setting data for screen: $screenId")
        database.child("screens").child(screenId).setValue(data)
            .addOnSuccessListener {
                android.util.Log.d("FirebaseService", "Data set successfully for: $screenId")
            }
            .addOnFailureListener { e ->
                android.util.Log.e("FirebaseService", "Failed to set data for: $screenId", e)
            }
    }
}
